// src/components/AddAuctionModal.tsx
...
import { useAddAuctionForm } from '../hooks/useAddAuctionForm'
...
const DescriptionField: React.FC<DescProps> = ({ label, ...props }) => (
  <FieldWrapper focused={false} filled={false}>
    <StyledLabel>{label}</StyledLabel>
    <StyledTextarea filled={false} {...props} />
  </FieldWrapper>
)
...
const AddAuctionModal: React.FC<Props> = ({ onClose }) => {
  const fileRef = useRef<HTMLInputElement | null>(null)
  const [preview, setPreview] = useState<string | null>(null)

  const { control, errors, onSubmit, loading, error } = useAddAuctionForm(
    (newAuction: AuctionType) => {
      onClose()
      window.location.reload();
      console.log('Ustvarjena dražba:', newAuction)
    }
  )
